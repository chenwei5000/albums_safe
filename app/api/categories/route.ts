import { bindings, jsonError, listJson, type Category } from '@/lib/edge-storage';

export async function GET() {
  const categories = await listJson<Category>('category:');
...
export async function POST(request: Request) {
  ...
  const existing = await listJson<Category>('category:');
  ...
  await bindings.ALBUM_KV.put(`category:${category.id}`, JSON.stringify(category));
